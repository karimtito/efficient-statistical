...
def read_tf_net(net_file, shape, is_trained_with_pytorch=False, batch_mode=False):
    mean = 0.0
    std = 0.0
    net = open(net_file,'r')
    ...
    if type(shape) in {int, np.int, np.int32, np.int64, tf.int32, tf.int64}:
        
        if batch_mode:
            x = tf.placeholder(tf.float64, [None,shape], name = "x")
        else:
    ...
    while True:
        ...
        if 'Normalize' in curr_line:
            ...
            b = myConst(b)
            if not batch_mode:
                temp_x = tf.reshape(x, [1, numel(x)])
            else:
                temp_x = tf.reshape(x, [tf.shape(x)[0], numel(x[0])])
            if(curr_line=="Affine"):
            
                x = tf.nn.bias_add(tf.matmul(temp_x,W), b)
            elif(curr_line=="ReLU"):
                x = tf.nn.relu(tf.nn.bias_add(tf.matmul(temp_x,W), b))
            elif(curr_line=="Sigmoid"):
                x = tf.nn.sigmoid(tf.nn.bias_add(tf.matmul(temp_x,W), b))
            elif(curr_line=="LeakyRelu"):
                x = tf.nn.leaky_relu(tf.nn.bias_add(tf.matmul(temp_x,W), b))
            elif(curr_line=="Sign"):
                x = tf.math.sign(tf.nn.bias_add(tf.matmul(temp_x,W), b))
            else:
                x = tf.nn.tanh(tf.nn.bias_add(tf.matmul(temp_x,W), b))
            ...
            print("MaxPool", args)
            if not batch_mode:
                temp_x = tf.reshape(x, [1] + args["input_shape"])
            else:
                temp_x = tf.reshape(x, [tf.shape(x)[0]]+ args["input_shape"])
            x = tf.nn.max_pool(temp_x, padding=padding_arg, strides=stride, ksize=ksize)
            print("\tOutShape: ", x.shape)
            ...
            if not batch_mode:
                ...
                temp_x = tf.reshape(x, [tf.shape(x)[0]] + args["input_shape"])
            x = tf.nn.conv2d(temp_x, filter=W, strides=stride_arg, padding=padding_arg)
